...
def test_embed_is_explicitly_hashed_at_sdk_surface() -> None:
    record = b"test-record"
    state = closure.embed(record)
    hashed = np.array(
        closure_rs.closure_element_from_raw_bytes("Sphere", [record], hashed=True)
    )
    geometric = np.array(
        closure_rs.closure_element_from_raw_bytes("Sphere", [record], hashed=False)
    )

    np.testing.assert_allclose(state.element, hashed, atol=1e-12)
    assert not np.allclose(state.element, geometric, atol=1e-12)
