...
impl Table {
    ...
    fn load_history_meta(dir: &Path) -> io::Result<HistoryMeta> {
        ...
        if !meta_path.exists() {
            return Self::create_history_files(dir);
        }
        let bytes = fs::read(meta_path)?;
        let meta = serde_json::from_slice(&bytes)
            .map_err(|e| io::Error::new(io::ErrorKind::InvalidData, format!("invalid history meta: {e}")))?;
        Ok(meta)
    }
    ...
    fn append_history_entry(
        ...
        let mut file = OpenOptions::new()
            .create(true)
            .append(true)
            .open(Self::history_oplog_path(&self.dir))?;
    ...
    pub fn live_row_count(&self) -> usize {
        self.row_tombstones.iter().filter(|&&deleted| !deleted).count()
    }
    ...
    /// Delete one row. O(1) algebraic identity update.
    /// Replaces row's quaternion with identity (contributes nothing).
    /// O(log n) delete via composition tree.
